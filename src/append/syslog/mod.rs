...
//! - RFC 3164, RFC 5424 and plain formats are supported.
...
//! - `max_len` &mdash; Maximum message length in bytes (all the headers included). Default is 2048. Please, keep in mind
//! that RFC 3164 forces message truncation to 1024 bytes.
//!
//! The default message format is `plain` which means that only the PRI part is added to a message that
//! you provide to the logger. If you want to use RFC 3164 or RFC 5424 format, you must provide `format` section
//! with `kind = rfc3164` or `kind = rfc5424` to your appender.
//!
//! RFC 3164/5424 format parameters:
//!
...
//! (the marker is used). This parameter only applied to RFC 5424 format.
...
impl Append for SyslogAppender {
    fn append(&self, record: &LogRecord) -> Result<(), Box<Error>> {
		let (msg, ml) = match self.msg_format {
		    MsgFormat::Plain(ref fmt)   => fmt.apply(&record),
		    MsgFormat::Rfc5424(ref fmt) => fmt.apply(&record),
			MsgFormat::Rfc3164(ref fmt) => fmt.apply(&record)
		};
		let eml = cmp::min(self.max_len, ml as usize); // effective min length
...
impl Deserialize for SyslogAppenderDeserializer {
    ...
    fn deserialize(&self, config: Value, deserializers: &Deserializers) -> Result<Box<Append>, Box<Error>> {
        ...
        if let Some(format) = config.format {
			...
			} else if format.kind == "rfc3164" {
	                builder.format(MsgFormat::Rfc3164(try!(deserializers.deserialize("format", &format.kind, format.config))));
